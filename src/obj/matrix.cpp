...
class Matrix {
 ...
 public:
  uint64_t get_offset(const uint64_t &dimension, const uint64_t index) {
    return index * dims + dimension;
  }
  double *data;

 public:
  Matrix(uint64_t amount_to_archive = 5, uint64_t dimensions = num_dims)
      : timesteps_stored(amount_to_archive), dims(dimensions) {
  ...
  void retrieveAtPoint(uint64_t &dimension, uint64_t &index,
                       double &output_data) {
    output_data = data[get_offset(dimension, index)];
  }
  void retrieveAtPoint(string &dimensionl, uint64_t &index,
                       double &output_data) {
    uint64_t dimension;
    if (dimensionl == "x") {
      dimension = 0;
