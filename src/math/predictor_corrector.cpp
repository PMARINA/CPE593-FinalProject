...
/*
The predictor corrector will need the inital 4 steps to begin calculations yn,
yn-1, yn-2, yn-3

We need to find a way to give each function to this (ie: velocity for each body)
and it will calculate the next step value
*/

/* double function(double t, double p){ //function that gives derivative of p at
time t
} */
...
void corrector(double &initial_value, double &step, double &fn0, double &fn1,
               ...
               double &return_to) {  // corrector: fn0 = fn, fn1 = fn-1, fn2 =
                                     // fn-2, f1 = fn+1
  return_to = (initial_value +
               ((step / 24.0) * (fn2 - 5.0 * fn1 + 19.0 * fn0 + 9.0 * f1)));
}
